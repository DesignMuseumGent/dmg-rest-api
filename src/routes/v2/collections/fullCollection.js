// this endpoint exposes the full collection of objects registred.
...
export function requestPrivateObjects(app, BASE_URI) {
    const privateObjectsHandler = async (req, res) => {
        ...
        if (!keys.some(item => item.key === apiKey)) {
            return res.status(401).json({
                error: 'Authentication key is missing or invalid. This stream is only available via authentication.'
            })
        }
        ...
        try {
            ...
            if (searchQuery) countQuery = countQuery.textSearch('search_vector', searchQuery, { type: 'websearch', config: 'dutch' })

            // build data query
            let selectFields
            if (!fullRecord) {
                selectFields = 'objectNumber, object_title_nl, iiif_manifest'
            } else if (showColors) {
                selectFields = 'objectNumber, json_ld_v2, object_title_nl, object_title_fr, object_title_en, object_description_nl, object_description_fr, object_description_en'
            } else {
                selectFields = 'objectNumber, json_ld_v2, object_title_nl, object_title_fr, object_title_en, object_description_nl, object_description_fr, object_description_en'
            }
            ...
            if (searchQuery) dataQuery = dataQuery.textSearch('search_vector', searchQuery, { type: 'websearch', config: 'dutch' })
            ...
            if (countError) {
                ...
                return res.status(500).json({ error: 'Error fetching private objects', details: countError.message })
            }

            if (error) {
                console.error('Fetch error:', JSON.stringify(error, null, 2))
                return res.status(500).json({ error: 'Error fetching private objects', details: error.message })
            }

            const totalPages = Math.ceil(count / itemsPerPage)
            const collectionId = `${BASE_URI}id/private/objects`
    ...
    app.get('/id/private/objects', privateObjectsHandler)
}