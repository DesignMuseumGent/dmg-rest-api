...
export function requestByColor(app, BASE_URI) {
    ...
    app.get('/v1/color-api/:color', async (req, res) => {
        ...
        res.status(200).send({
            ...
            "@id": `${BASE_URI}id/color-api/${targetColor}?image=true`,
            "hydra:totalItems": matchingObjects.length,
            "hydra:view": {
                "@id": `${BASE_URI}id/color-api/${targetColor}?image=true&pageNumber=${pageNumber}`,
                "@type": "PartialColletionView",
                "hydra:first": `${BASE_URI}id/color-api/${targetColor}?image=true&pageNumber=1`,
                "hydra:previous": previousPage? `${BASE_URI}id/color-api/${targetColor}?image=true&pageNumber=${previousPage}` : null,
                "hydra:next": nextPage? `${BASE_URI}id/color-api/${targetColor}?image=true&pageNumber=${nextPage}` : null,
                "hydra:last": `${BASE_URI}id/color-api/${targetColor}?image=true&pageNumber=${totalPages}`,
            },
